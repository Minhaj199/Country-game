...
export function AnswerButton({ country, disabled, isCorrect, isSelected, onPress }: AnswerButtonProps) {
  const theme = useTheme();
  const resultColor = isCorrect ? theme.colors.primary : '#DC2626';
  const backgroundColor = isCorrect ? '#DCFCE7' : isSelected ? '#FEE2E2' : theme.colors.surface;
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.button,
        { backgroundColor, borderColor: isCorrect || isSelected ? resultColor : theme.colors.outline },
        pressed && !disabled && styles.pressed,
      ]}
    >
      <Text style={styles.label} variant="titleMedium">{country.name}</Text>
